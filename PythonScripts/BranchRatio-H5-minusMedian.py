...
#locate the metadata
import dendropy
ds=dendropy.DataSet.get_from_path("/Volumes/Xueting2019/0617-serverdownload-needed/0603-H5-Egypt-all-branchrates/comb/branchRatio-comparedtoMedian/H5outfileWithMedian.tree", "nexus", extract_comment_metadata=True)
    ...
    for tree in trees:
        ...
        for nd in tree:
            ...
            if x1 is None or x2 is None:
                continue;
            nd.HeadRatiodif=float(x1)/float(x2)-0.4622
            nd.annotations.add_bound_attribute("HeadRatiodif")
            #print(nd.Head)
            y1=nd.annotations.get_value("Stalk1+2.rate_median")
            y2=nd.annotations.get_value("Stalk3.rate_median")
            if y1 is None or y2 is None:
                continue;
            nd.StalkRatiodif=float(y1)/float(y2)-0.1503
            nd.annotations.add_bound_attribute("StalkRatiodif")  
            #print(tree.as_string("nexus")) 
            ds.write_to_path('H5outfile-comparedtoMedian.tree','nexus',
